...
KNOWN_AGENTS = ["tool", "research", "rag", "supervisor", "brainstorm", "graph_rag", "general"]


def get_agent_metrics() -> dict:
    """Get per-agent execution metrics."""
    agents = {
        name: {"calls": 0, "success": 0, "errors": 0, "avg_latency_ms": 0.0, "success_rate_pct": 100.0}
        ...
        if raw:
            for key, value in raw.items():
                ...
                if agent_name not in agents:
                    agents[agent_name] = {"calls": 0, "success": 0, "errors": 0, "avg_latency_ms": 0.0, "success_rate_pct": 100.0}
            ...
            for name, data in agents.items():
                total = data["calls"]
                if total > 0:
                    data["avg_latency_ms"] = round(data.pop("total_latency", 0) / total, 2)
                    data["success_rate_pct"] = round(data["success"] / total * 100, 1)
